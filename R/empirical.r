#'Compute empirical distribution of symptom duration
#'
#'This function computes the empirical distributions of symptom duration from a subset of the weekly survey.
#'The subset can be set either with \code{subset} or with a vector of warnings to be removed (\code{remove_warnings}). 
#'Finally, covariates can be summarized for each symptom duration with \code{summarize_covariates}. See example below.
...
#' @param  summarize_covariates named character vector. Specify how to summarize covariates for each episode of illness. The names correspond to the summarized covariate variable in the outputted data frame.
...
#' summarize_severity <- c("symptom_severity"="max(na.omit(symptom_severity))")
#' empirical_distribution_symptom_duration(flunet, remove_warnings=bad_warnings, summarize_covariates=summarize_severity)
#'}
empirical_distribution_symptom_duration <- function(flunet, subset=NULL, remove_warnings=NULL, summarize_covariates=NULL) {

	if(is_survey_present(flunet,survey="weekly",warning_message="")){
	...
	df_subset <- subset(df_weekly,((!is.na(symptom_start) & position_bout == 1) | (!is.na(symptom_end) & position_bout == length_bout)) & eval(parse(text=subset))) %>%
	match_df(df_weekly,.,c("person_id","n_bout"))  %>% 
	group_by(person_id,n_bout) %>% 
	arrange(comp_time)

	# summarize covariates
	call_summarize <- parse(text=sprintf("summarize(df_subset,symptom_start=first(symptom_start),symptom_end=last(symptom_end),%s)",paste(paste(names(summarize_covariates),summarize_covariates,sep="="),collapse=",")))	
	df_summarize <- eval(call_summarize)
	
	# compute symptom duration
	ans <- df_summarize %>% mutate(symptom_duration=symptom_end - symptom_start) %>% filter(!is.na(symptom_duration))

	# remove some variables
	ans <- ans[setdiff(names(ans),c("n_bout","symptom_start","symptom_end"))]
	
	flunet$log$empirical_distributions$symptom_duration <- ans
